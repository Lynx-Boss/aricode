...
Repository: \url{https://gitlab.com/ariproyect/aricoderoot} (commit \texttt{3e571ae}).
}}\vspace{1em}

% ---- Abstract ----
\begin{abstract}
We present \textsc{Aricode}, a statically typed, compiled programming language that makes silent runtime errors structurally impossible at compile time. Unlike existing approaches that rely on programmer discipline (Go), optional annotations (Python), or complex type systems (Rust), Aricode enforces a five-level error classification where code that can fail silently is \emph{rejected by the compiler}. The language features direct x86\_64 machine code generation without intermediate representations, linkers, or runtime libraries, producing minimal ELF binaries---199 bytes for Hello World, 674 bytes for a prime number generator. We introduce four novel contributions: (1) a mandatory error hierarchy that classifies all possible failures into five severity levels with distinct compilation semantics, (2) a compile-time performance rating system (Podium) that evaluates generated machine code quality, (3) native arbitrary-precision decimal arithmetic that eliminates IEEE~754 floating-point errors, and (4) a compiler that implements constant folding, strength reduction, peephole optimization, and tail call optimization while generating binaries 2{,}000$\times$ smaller than GCC. Benchmarks across twelve computational challenges---spanning arithmetic, recursion, number theory (Collatz conjecture, Mersenne prime M31), cryptographic primitives (modular exponentiation), and artificial intelligence (perceptron training, minimax game tree search)---demonstrate that Aricode competes with hand-optimized x86\_64 assembly across all categories. In an overall ranking of 84 binaries across 8 implementations, Aricode and NASM assembly tie at the top, both approximately $2\times$ ahead of GCC~-O3.
\end{abstract}
...
We designed twelve challenges of increasing computational complexity spanning five categories:

\begin{table}[H]
\centering
\caption{Benchmark challenge suite (12 challenges, 5 categories)}
...
\begin{table}[H]
\centering
\caption{Overall sparring ranking --- 12 challenges, 84 binaries}
\label{tab:ranking}
\begin{tabular}{@{}clr@{}}
\toprule
\textbf{Rank} & \textbf{Implementation} & \textbf{Points} \\
\midrule
\textbf{1} & \textbf{NASM x86\_64 assembly} & \textbf{264} \\
\textbf{1} & \textbf{Aricode} & \textbf{257} \\
3 & C (gcc -O3) & 144 \\
4 & C (gcc -O2) & 141 \\
5 & C (gcc -Os) & 138 \\
6 & C (gcc -O0) & 126 \\
7 & Python 3.12 & 115 \\
8 & C (gcc -O2 dynamic) & 111 \\
\bottomrule
\end{tabular}
\end{table}

Aricode and hand-written NASM assembly effectively tie at the top (264 vs.\ 257 points), with Aricode winning in binary size across all 12 challenges and NASM winning in execution speed for recursion-heavy benchmarks. Both are approximately $2\times$ ahead of GCC~-O3 (144 points), which is penalized by large binary sizes (754~KB for programs that Aricode compiles to 157--1{,}576 bytes). Aricode wins outright in the cryptographic (powmod) and algorithmic (primecount) categories, demonstrating that its while-loop codegen with strength reduction generates tighter loops than GCC's optimized output.
...
Current limitations include: the codegen targets only Linux x86\_64; the type system lacks user-defined types (structs, enums); floating-point types (f32, f64) are not yet implemented; and the standard library is limited to \texttt{print\_int} and \texttt{print\_str} builtins. Tail call optimization currently applies only to self-recursive calls, not mutual recursion. Bitwise operators (\texttt{\&}, \texttt{|}, \texttt{\^{}}, \texttt{<<}, \texttt{>>}) are fully supported in both the parser and code generator.
