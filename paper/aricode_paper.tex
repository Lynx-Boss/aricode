...
\begin{table}[H]
\centering
\caption{Overall sparring ranking --- 14 challenges, 126 binaries, 10 implementations}
\label{tab:ranking}
\begin{tabular}{@{}clrl@{}}
\toprule
\textbf{Rank} & \textbf{Implementation} & \textbf{Points} & \textbf{vs.\ Aricode} \\
\midrule
1 & NASM x86\_64 assembly & 392 & 1.02$\times$ \\
\textbf{2} & \textbf{Aricode} & \textbf{383} & \textbf{1.00$\times$} \\
3 & C (gcc -O2) & 255 & 0.67$\times$ \\
4 & C (gcc -O3) & 243 & 0.63$\times$ \\
5 & C (gcc -Os) & 239 & 0.62$\times$ \\
6 & C (gcc -O0) & 239 & 0.62$\times$ \\
7 & C (gcc -O2 dynamic) & 212 & 0.55$\times$ \\
8 & Python 3.12 & 163 & 0.43$\times$ \\
9 & Rust 1.85 & 100 & 0.26$\times$ \\
10 & Go 1.24 & 84 & 0.22$\times$ \\
\bottomrule
\end{tabular}
\end{table}
...
Current limitations include: the codegen targets only Linux x86\_64; the type system lacks first-class struct declarations (structs are implemented via array patterns with constructor functions); string operations are limited to literal printing; and there is no module or import system. Tail call optimization applies only to self-recursive calls. The standard library consists of seven builtins: \texttt{print\_str}, \texttt{print\_int}, \texttt{print\_float}, \texttt{read\_int}, \texttt{float\_to\_int}, \texttt{arr\_new}/\texttt{arr\_get}/\texttt{arr\_set}/\texttt{arr\_len}.
