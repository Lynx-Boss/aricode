...
Repository: \url{https://gitlab.com/ariproyect/aricoderoot}.
}}\vspace{1em}
...
Current limitations include: the codegen targets only Linux x86\_64; the type system lacks first-class struct declarations (structs are implemented via array patterns with constructor functions); and circular imports are not detected (would cause infinite recursion during compilation). Tail call optimization applies only to self-recursive calls. Decimal arithmetic is currently compile-time only (constant folding); runtime decimal operations would require embedding the BCD runtime in the binary. The standard library consists of 23~builtins spanning six categories: console I/O (\texttt{print\_str}, \texttt{print\_int}, \texttt{print\_float}, \texttt{print\_dec}, \texttt{read\_int}, \texttt{read\_float}), strings (\texttt{str\_new}, \texttt{str\_len}, \texttt{str\_eq}, \texttt{str\_char\_at}, \texttt{str\_println}, \texttt{str\_concat}), arrays (\texttt{arr\_new}, \texttt{arr\_get}, \texttt{arr\_set}, \texttt{arr\_len}), files (\texttt{file\_open}, \texttt{file\_read}, \texttt{file\_write}, \texttt{file\_close}), and type conversion (\texttt{int\_to\_float}, \texttt{float\_to\_int}, \texttt{dec}). The language includes a multi-file compilation system via \texttt{import} statements with automatic dependency resolution.

Future work includes: ARM64 and WebAssembly backends; first-class struct and enum types; a package manager; SIMD vectorization for numeric workloads; circular import detection; and inter-procedural optimization.
...
The direct x86\_64 code generator, combined with constant folding, peephole optimization, and tail call optimization, produces binaries that are 2{,}000$\times$ smaller than GCC output and execute 1.2$\times$ faster than GCC~-O3. In a comprehensive benchmark suite including Ackermann function evaluation, Collatz conjecture verification, and Mersenne prime testing, Aricode achieves second place overall among twelve implementations, just 7~points behind hand-optimized x86\_64 assembly.

The compiler, benchmark suite, and all source code are available at: \url{https://gitlab.com/ariproyect/aricoderoot}.
